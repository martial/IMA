...
ofVec2f ofxDragAndZoomable::getPointAtCoordinate(ofVec2f point, ofRectangle r) {
    
    float s = origRect.width / width;
    
    ofVec2f pnt;
    pnt.x = (point.x * s) - ((x + r.getPosition().x) * s) ;
    pnt.y = (point.y * s) - ((y + r.getPosition().y) * s) ;
...
void ofxDragAndZoomable::scaleToOrigin() {
    
    this->x = ofGetWidth() *.5 - origRect.x*.5;
    this->y = ofGetHeight() *.5 - origRect.y*.5;
    
    this->width = origRect.width;
    this->height = origRect.height;
}